...
Data and [R](https://www.r-project.org/) code for the analysis supporting this [Oct. 23, 2018 BuzzFeed News post](https://www.buzzfeednews.com/article/peteraldhous/russia-online-trolls-engagement-strategy) analyzing engagement with English-language tweets from the Kremlin-backed Internet Research Agency. Supporting files are in [this GitHub repository](https://github.com/BuzzFeedNews/2018-10-russian-troll-tweets).
...
Run the script `data_prep.R` to download both datasets, filter the Twitter data for English langauge tweets only, join the Clemson account categories to the Twitter data, and save as the file `en_ira_tweets.csv` in the `data` folder.
...
- `tweet_text` Yext of the tweet (mentions of anonymized accounts have been replaced with anonymized userid).
- `tweet_time` Time when the tweet was published (UTC).
- `tweet_client_name` Name of the client app used to publish the tweet.
- `in_reply_to_tweetid` Tweetid of the original tweet that this tweet is in reply to (for replies only).
- `in_reply_to_userid` Userid of the original tweet that this tweet is in reply to (for replies only).
- `quoted_tweet_tweetid` Tweetid of the original tweet that this tweet is quoting (for quotes only).
...
- `user_mentions` List of userids who are mentioned in this tweet (includes anonymized userids).
...
^2^ These engagement counts exclude engagements from userssuspended, deleted or otherwise sanctioned by Twitter at the time of the data release.
...
datatable(top_tweets, colnames = c("Display name", "Screen name", "Account category", "Profile description", "Tweet text", "Retweet count", "Date"))
```
...
### How many tweets, by account category, were retweets of other Russian troll tweets?

```{r, warning=FALSE, message=FALSE}
retweet_troll_ids <- intersect(unique(en_ira_tweets$retweet_userid),unique(en_ira_tweets$userid))

en_ira_tweets %>%
  filter(is_retweet == "true" & retweet_userid %in% retweet_troll_ids) %>%
  count()
```
